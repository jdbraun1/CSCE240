...
bool Match(string input, string ToMatch)
{
	...
	else if(percentMatch >= threshold)
	{
		ofstream out_myfile(out_file_name, std::ios_base::app);
...
bool MatchQuery(string query)
{
	...
	else if(Match(query, "What district do you support for Q/A") || Match(query, "What district do you support"))
	{
		GetInfo("Contact Information:region");
		return true;
	}
	else
	{
...
string CreateChatFile()
{
	...
	nn<<myfilename<<"data/chat_sessions/"<<date<<"_"<<time<<".txt";
	ofstream myfile;
	myfile.open(nn.str());
	myfile << "hello";
	myfile.close();
	...
	return "data/chat_sessions/"+date+"_"+time+".txt";
}

//method for command line arguments
void csvFile(string command, int chatnumber)
{
	ifstream input_csv("data/chat_statistics.csv");
			...
			input_csv.close();
			//command line argument to just show summary
			if(strcasecmp(command.c_str(),"summary") == 0)
			{
			...
			//Using command line argument to showchat summary
			else if(strcasecmp(command.c_str(),"showchat-summary") == 0)
			{
				bool found = false;
				int chatNumber = chatnumber;
				string currentSess;
				int chatCount = 0;
				...
				if(found == false)
				{
					cout << "Error session not found there are only " << chatCount-1 <<" chat sessions.\nPlease choose a valid number" << endl;
				}
			}

			//shows the whole chat
			else if(strcasecmp(command.c_str(),"showchat") == 0)
			{
				bool found = false;
				int chatNumber = chatnumber;
				string currentSess;
	...
	else
	{
		...
		else if(strcmp(argv[1],"showchat-summary") == 0)
		{
			try
			{
				...
				csvFile("showchat-summary", chatNumber);
			}
			catch(const invalid_argument& ia)
			{
		...
		//shows the whole chat asked for
		else if(strcmp(argv[1],"showchat") == 0)
		{
			try
			{
				int chatNumber = stoi(argv[2]);
				csvFile("showchat", chatNumber);
			}
			catch(const invalid_argument& ia)
			{
				cout << "Invalid command line argument" << endl;
